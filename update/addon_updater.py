...
class SingletonUpdater:
    ...
    def get_tags(self):
        ...
        if self._tags is None:
        ...
        elif self._prefiltered_tag_count == 0 and not settings.update_check_commits:
            self._tag_latest = None
        ...
        elif self._prefiltered_tag_count == 0 and settings.update_check_commits:
            if not self._error:
            ...
            self.print_verbose("{} branch found, no releases: {}".format(
                ...
                and settings.update_check_commits)
                or (len(self._tags) == 0 and not settings.update_check_commits)
            ...
            if not settings.update_check_commits:
                if str(self._tags[0]['name']).find("beta") == -1 and str(self._tags[0]['name']).find("alpha") == -1:
                    self._tag_latest = self._tags[0]
                    self.print_verbose(
                ...
                if str(self._tags[n]['name']).find("beta") == -1 and str(self._tags[n]['name']).find("alpha") == -1:
                    self._tag_latest = self._tags[n]  # guaranteed at least len()=n+1
                    self.print_verbose(
                        "Most recent commit with tag found:" + str(self._tags[n]['name']))
    ...
    def deep_merge_directory(self, base, merger, clean=False):
        ...
        for path, dirs, files in os.walk(base):
            if os.path.basename(path) == "scripts":
                self.print_verbose("Ignoring scripts folder...")
                continue
    ...
    def version_tuple_from_text(self, text):
        ...
        segments = list()
        tmp = ''
        if str(text).find("beta") == -1 and str(text).find("alpha") == -1:
