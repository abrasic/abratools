...
class AddonUpdaterCheckNow(bpy.types.Operator):
    ...
    bl_description = "Check for updates by connecting to the GitHub repository. Requires internet connection"
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateNow(bpy.types.Operator):
    ...
    bl_description = "Update to the latest version of the {x} addon. Anything inside of the 'scripts' folder and preferences will NOT be removed".format(
        x=updater.addon)
    bl_options = {'REGISTER', 'INTERNAL'}
...
class AddonUpdaterUpdateTarget(bpy.types.Operator):
    ...
    bl_description = "Install a specific release of abraTools from GitHub"
    bl_options = {'REGISTER', 'INTERNAL'}
    ...
    def target_version(self, context):
        ...
        for tag in updater.tags:
            if tag.find("beta") == -1 and tag.find("alpha") == -1:
                tag_label = tag
                tag_desc = "Install version " + tag_label
                if tag == "Main":
                    tag_label = "Latest Branch"
                    tag_desc = "This will install the latest code published from the 'main' branch on the GitHub repository. Only use if you're debugging or want to try out new/experimental features."
                ret.append((tag, tag_label, tag_desc))
                i += 1
        return ret
    ...
    def draw(self, context):
        ...
        if updater.invalid_updater:
            ...
            return
        layout.label(text="Install a specific version of abraTools from GitHub")
        layout.separator()
...
class AddonUpdaterRestoreBackup(bpy.types.Operator):
    ...
    bl_description = "Restore abraTools from a local backup"
    bl_options = {'REGISTER', 'INTERNAL'}

    @classmethod
    def poll(cls, context):
        try:
            return os.path.join(updater.stage_path, "backup")
        except:
            return False
...
def background_update_callback(update_ready):
    ...
    #     updater.print_verbose("Popups off")
    #     return
    if not update_ready:
        updater.print_verbose("Not ready")
        return
...
def update_settings_ui(self, context, element=None):
    ...
    # elif settings.update_check_commits and \
    ...
    #                    text="", icon="FILE_REFRESH")
        ...
        split.operator(AddonUpdaterUpdateNow.bl_idname,
                       text="Update available (" +str(ver_tuple[0])+"."+str(ver_tuple[1])+"."+str(ver_tuple[2])+")")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="You're up-to-date ("+str(ver_tuple[0])+"."+str(ver_tuple[1])+"."+str(ver_tuple[2])+")", icon="CHECKMARK")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
    ...
    if not updater.manual_only:
        ...
        col.scale_y = 1
        col.prop(settings, "update_check_commits")
        col.separator()
        if settings.update_check_commits and len(updater.include_branch_list) > 0:
            col.operator(AddonUpdaterUpdateTarget.bl_idname,
                         text="Install version...")
        else:
            col.operator(AddonUpdaterUpdateTarget.bl_idname,
                         text="Re-install version...")
        ...
        if "backup_date" in updater.json and os.path.isdir(backup_path):
            if updater.json["backup_date"] == "":
                ...
                col2.enabled = True
        backup_text = "Restore from backup... ({})".format(last_date)
        col2.operator(AddonUpdaterRestoreBackup.bl_idname, text=backup_text)
        col3 = col.column(align=True)
        col3.operator("wm.url_open", text="Install manually from GitHub", icon="URL").url = "https://github.com/abrasic/abratools"
    ...
    #     row.label(text="Last checked " + last_check)
    # else:
    #     row.label(text="")
...
def skip_tag_function(self, tag):
    ...
    if settings.update_check_commits:
